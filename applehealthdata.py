...
class HealthDataExtractor(object):
    ...
    def open_for_writing(self):
        ...
        for kind in (list(self.record_types) + list(self.other_types)):
            ...
            if WRITE: f.write(','.join(FIELDS[headerType].keys()) + '\n')
            self.handles[kind] = f
        ...
        for node in self.nodes:
            if node.tag in kinds:
                ...
                for (field, datatype) in FIELDS[node.tag].items():
                    if field not in ['totalDistance', 'totalDistanceUnit', 'totalEnergyBurned', 'totalEnergyBurnedUnit', 'gpxFilePath']:
                        values.append(format_value(attributes.get(field), datatype))

                workout_stats_tags = node.findall("WorkoutStatistics")
                distance_tag = [tag for tag in workout_stats_tags if tag.attrib['type'] == 'HKQuantityTypeIdentifierDistanceWalkingRunning']
                cal_burned_tag = [tag for tag in workout_stats_tags if tag.attrib['type'] == 'HKQuantityTypeIdentifierActiveEnergyBurned']



                if distance_tag:
                    distance_tag = distance_tag[0]
                    distance = distance_tag.attrib['sum']
                    distance_unit = distance_tag.attrib['unit']
                    values.append(format_value(distance, 'n'))
                    values.append(format_value(distance_unit, 's'))
                else:
                    values.append("")
                    values.append("")

                if cal_burned_tag:
                    cal_burned_tag = cal_burned_tag[0]
                    cal_burned = cal_burned_tag.attrib['sum']
                    cal_burned_unit = cal_burned_tag.attrib['unit']
                    values.append(format_value(cal_burned, 'n'))
                    values.append(format_value(cal_burned_unit, 's'))
                else:
                    values.append("")
                    values.append("")

                workout_route_tag = node.find('WorkoutRoute')
                if workout_route_tag:
                    gpx_file_ref = workout_route_tag.find('FileReference')
                    values.append(format_value(gpx_file_ref.attrib['path'], 's'))

                line = ','.join(values) + '\n'
                if WRITE: self.handles[kind].write(line)
